...
public class SnapshotCodecTest {
  ...
  public void testMinimalDeserilization() throws Exception {
    ...
    assertEquals(txVisibilityState, txVisibilityState2);
    assertEquals(readPtr, txVisibilityState2.getReadPointer());
    try {
  ...
  private void assertTransactionVisibilityStateEquals(TransactionVisibilityState expected,
    ...
    assertEquals(expected.getTimestamp(), input.getTimestamp());
    assertEquals(expected.getReadPointer(), input.getReadPointer());
    assertEquals(expected.getWritePointer(), input.getWritePointer());
    assertEquals(expected.getInProgress(), input.getInProgress());
    assertEquals(expected.getInvalid(), input.getInvalid());
  }
}
